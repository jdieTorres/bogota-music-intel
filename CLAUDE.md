...
- **Sacar la key gratuita de Last.fm** antes de la Fase 5. Es la mitad del radar de tendencias y la única fuente con popularidad *por país* (`geo.gettopartists?country=colombia`); Deezer no tiene ese eje. El servicio está vivo y el alta es self-service (verificado 2026-08-28).
...
**Lo siguiente es la Fase 5 (radar de tendencias), y lo único que la frena es una key.** Las fuentes quedaron verificadas llamándolas el 2026-08-28 (ver `docs/…` § 2.2), y son dos ejes distintos que se complementan:
- **Deezer**, sin key y funcionando: la editorial **"Música colombiana" (id 498)**. Contesta *qué es música colombiana*. No existe chart por país, contra lo que decía el doc.
- **Last.fm**, viva y self-service pero **sin key sacada todavía**: `geo.gettopartists?country=colombia`. Contesta *qué se escucha en Colombia*, que no es lo mismo.

Al construirla, anticipar que la editorial de Deezer mezcla nacionalidades (Bad Bunny aparece en "Música colombiana"): va a pedir el mismo tipo de filtro editorial que ya tiene la cartelera.

Después viene la Fase 6 (pulido y deploy), que **incluye la pasada final de look & feel** — no se despliega sin ella.
