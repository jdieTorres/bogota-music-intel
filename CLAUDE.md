...
### Nuevo pendiente que abrió el filtrado editorial
- **Lista curada de artistas locales.** MusicBrainz resuelve bien al internacional consagrado y mal al local emergente: de los 58 eventos solo 3 quedaron confirmados como locales, y artistas colombianos reales como El Kalvo, PABLOPABLO, LA MUCHACHA o MADE4RAP quedaron sin resolver (El Kalvo figura en MusicBrainz **sin país**). Mientras eso siga así, "priorizar lo local" destaca muy poco. La vía es una lista curada con evidencia, mismo patrón que `coordenadas_curadas.py`. **Es trabajo de conocimiento de escena, no técnico — le toca a Juan decidir a quién entra.**
