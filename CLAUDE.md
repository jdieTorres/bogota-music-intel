...
- **El mapa no se ha mirado con las 9 salas puestas.** Desde el 2026-08-27 están las 9 de 9 ubicadas (antes 5), pero lo verificado es que los datos llegan a la página: `/mapa` responde, nombra las 9 salas y ya no renderiza la sección "sin ubicar". **Los pines se dibujan en canvas del lado del cliente, así que nada de eso demuestra que se vean** — es exactamente el escenario del mapa en negro con todo en verde.
...
- **Paleta oscura fija** en `apps/web/src/app/globals.css` (tokens `--background`, `--surface`, `--accent`…), elegida porque la cartelera se consulta de noche y el color lo deben poner los afiches. Es el punto de partida del trabajo de look & feel, no una decisión cerrada.
  - **El mapa es la excepción, y es deliberada.** Desde el 2026-08-27 usa el estilo claro `liberty` de OpenFreeMap, elegido por Juan tras mirar cuatro en el navegador (`dark`, `fiord`, `liberty`, `bright`). El motivo: un mapa casi negro leía como un hueco en la página en vez de como un mapa. Es la primera decisión del trabajo de look & feel; si la paleta general cambia, esto se revisa **junto con ella**, no por su cuenta.
