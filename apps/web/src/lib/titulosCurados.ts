 ...
 *
 * Es el equivalente de presentación de `artistas_locales.py` y
 * `coordenadas_curadas.py`: lo que no se puede deducir se cura a mano, con
 * evidencia, en vez de ensanchar una regla hasta que atrape el caso. Las
 * reglas de `tituloEvento.ts` corrigen lo que es estructura (mayúsculas
 * gritadas, "en Bogotá" al final, varios artistas separados por barras);
 * acá va solo lo que exige saber algo que el título no dice.
 *
 ...
 *    engancharse — mismo riesgo que `eventos_excluidos.py`, y por eso solo
 ...
 *    fuente, que es feo pero no miente. Inventarle una gira a un evento es
 *    exactamente lo que la regla dura del proyecto prohíbe.
 */

/** Cómo se escribe un artista, cuando la fuente lo escribe mal. */
export type GrafiaCurada = {
  /** Tal como sale del título de la fuente, ya limpio de ruido. */
  comoLoPublican: string;
  /** Cómo se escribe de verdad. */
  nombre: string;
  evidencia: string;
};

export const GRAFIAS: GrafiaCurada[] = [
  {
    comoLoPublican: "5 Seconds of Summers",
  ...
  {
    comoLoPublican: "Slauhgter to Prevail",
    nombre: "Slaughter to Prevail",
    evidencia:
      "Errata del Royal Center, que publica 'SLAUHGTER TO PREVAIL'. Ya " +
      "estaba registrada en services/api/bogota_music_intel/artistas_locales.py " +
      "como `tambien_como` de 'Slaughter to Prevail'.",
  },
  {
    comoLoPublican: "Mad Profesor",
  ...
  },
  {
    ...
    evidencia:
      ...
      "services/api/bogota_music_intel/artistas_locales.py (Pablo Drexler, " +
      "escena alternativa española).",
  },
  {
    comoLoPublican: "Atake Mapale",
    nombre: "Atake Mapalé",
    evidencia:
      "Lleva tilde. Latino Power lo publica sin ella " +
      "('Mukangu/Atake Mapale/ Los Yoryis'). Ya está con tilde y con su " +
      "evidencia en services/api/bogota_music_intel/artistas_locales.py.",
  },
  {
    comoLoPublican: "Alvaro Diaz",
    nombre: "Álvaro Díaz",
    evidencia:
...
export type TituloCurado = {
  /** El título crudo, exacto como lo publica la fuente. */
  comoLoPublican: string;
  /** Uno o más artistas de cartel, ya bien escritos. */
  artistas: string[];
  /** El nombre de la gira o del show. `null` si de verdad no hay. */
  gira: string | null;
  evidencia: string;
};

export const TITULOS: TituloCurado[] = [
  {
    comoLoPublican: "Alvaro Diaz 2026",
  ...
  {
    comoLoPublican: "10 AÑOS Y NO AZARAN - LA MUCHACHA EN BOGOTÁ",
  ...
  {
    comoLoPublican: "RAYOS LASER ACÚSTICO EN BOGOTÁ",
  ...
  {
    comoLoPublican: "Lenny Tavarez – J quiles",
  ...
  {
    comoLoPublican: "Carlos Vives & La Provincia Tour Al Sol",
