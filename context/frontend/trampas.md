...
- **MapLibre le escribe estilos en línea a cada marcador, y un estilo en línea le gana a la hoja.** Le pone `transform` para posicionarlo y `opacity` para su propio desvanecido de pines tapados, así que **una regla de esas dos propiedades sobre `.marcador-sala` no hace nada**. Ya mordió dos veces: un `scale(1.15)` que no corrió entre el 2026-09-07 y el 2026-09-08 sin que nadie lo notara, y el 2026-09-15 la atenuación del pin de una sala sin eventos — con la clase puesta en el DOM, el CSS compilado y presente en el bundle, y `getComputedStyle` devolviendo 1. Para el `scale` la salida fue aplicarlo al `svg` de adentro, que MapLibre no toca. **Se detecta midiendo el estilo calculado, no comprobando que la clase esté.**
- **Y en ese pin la opacidad no es la herramienta, aunque se aplique.** Llevarla al `svg` sí funcionó y rompió otra cosa: **el pin dejó de tener forma.** El aro blanco se volvió translúcido mientras el `drop-shadow` del botón seguía entero, así que el marcador pasó de disco con borde a mancha difusa sin punto focal, y al alejar el mapa —con veinte pines amontonados en el centro— parecían mal ubicados. No lo estaban: medidos contra su proyección Mercator daban **0.0 px de error**. La lección, que el comentario de `.marcador-sala` ya anticipaba: **lo que sostiene la legibilidad de ese pin es el aro blanco y la sombra**, y la opacidad los desactiva de hecho. Para distinguir un estado se cambia el relleno (`--pin-fondo`), nunca la transparencia — con la clase repetida en el selector, porque `.marcador-sala` se define después y con la misma especificidad.
